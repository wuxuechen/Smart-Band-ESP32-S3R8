...
static uint8_t raw_adv_data[] = {
    ...
    /* Service UUID (example: 0x00FF) */
    0x03, 0x03, 0xFF, 0x00,

    /* Device name */
    (sizeof(DEVICE_NAME)), 0x09,  // length = type(1) + strlen(name)
    'T','U','P','_','G','A','T','T','S'
};
...
static bool bt_initialized = true;
...
void bt_switch(bool enable) {
    ...
    if (enable) {
        if (bt_initialized) {
            ESP_LOGI(GATTS_TABLE_TAG, "Bluetooth already enabled");
            return;
        }
        ...
        bt_initialized = true;
        ESP_LOGI(GATTS_TABLE_TAG, "Bluetooth enabled successfully");

    } else {
        if (!bt_initialized) {
            ESP_LOGI(GATTS_TABLE_TAG, "Bluetooth already disabled");
            return;
        }
        ...
        if (esp_bluedroid_get_status() == ESP_BLUEDROID_STATUS_ENABLED) {
            ...
            if (ret != ESP_OK) ESP_LOGE(GATTS_TABLE_TAG, "Bluedroid disable failed: %s", esp_err_to_name(ret));
        }

        // Deinit Bluedroid if initialized
        if (esp_bluedroid_get_status() == ESP_BLUEDROID_STATUS_INITIALIZED) {
            ret = esp_bluedroid_deinit();
            if (ret != ESP_OK) ESP_LOGE(GATTS_TABLE_TAG, "Bluedroid deinit failed: %s", esp_err_to_name(ret));
        }

        // Disable BT controller if enabled
        if (esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_ENABLED) {
            ret = esp_bt_controller_disable();
            if (ret != ESP_OK) ESP_LOGE(GATTS_TABLE_TAG, "Controller disable failed: %s", esp_err_to_name(ret));
        }

        // Deinit BT controller if initialized
        if (esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_INITED) {
            ret = esp_bt_controller_deinit();
            if (ret != ESP_OK) ESP_LOGE(GATTS_TABLE_TAG, "Controller deinit failed: %s", esp_err_to_name(ret));
        }

        bt_initialized = false;
        ESP_LOGI(GATTS_TABLE_TAG, "Bluetooth fully disabled");
    }
}
